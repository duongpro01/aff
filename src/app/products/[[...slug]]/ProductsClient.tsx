...
export default function ProductsClient({ products, brands, categories, currentCategory }: ProductsClientProps) {
  ...
  const filtered = useMemo(() => {
    ...
    return result;
  }, [products, selectedCategories, selectedBrands, minPrice, maxPrice, sortBy]);
  ...
  const filterSidebar = (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      {/* Danh muc */}
      <div>
        <h3 style={{ fontSize: 15, fontWeight: 700, marginBottom: 10, color: '#111827' }}>Danh mục</h3>
      ...
      {/* Thuong hieu */}
      <div>
        <h3 style={{ fontSize: 15, fontWeight: 700, marginBottom: 10, color: '#111827' }}>Thương hiệu</h3>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {brands.map((br: any) => (
            <label
              key={br.slug}
      ...
      {/* Khoang gia */}
      <div>
        <h3 style={{ fontSize: 15, fontWeight: 700, marginBottom: 10, color: '#111827' }}>Khoảng giá</h3>
    ...
    <div style={{ maxWidth: 1280, margin: '0 auto', padding: '24px 16px' }}>
      ...
      <nav style={{ marginBottom: 16, fontSize: 14, color: '#6b7280' }}>
        ...
        {currentCategory ? (
          <>
            ...
            <span style={{ margin: '0 8px' }}>/</span>
            <span style={{ color: '#111827', fontWeight: 500 }}>{currentCategory.name}</span>
          </>
        ) : (
          <span style={{ color: '#111827', fontWeight: 500 }}>Products</span>
        )}
      </nav>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        {currentCategory?.description && (
          <p style={{ marginTop: 8, color: '#6b7280', fontSize: 14 }}>{currentCategory.description}</p>
        )}
      </div>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          {paged.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '60px 20px', color: '#6b7280' }}>
              <p style={{ fontSize: 16, fontWeight: 500 }}>Không tìm thấy sản phẩm phù hợp</p>
              <p style={{ fontSize: 14, marginTop: 8 }}>Hãy thử thay đổi bộ lọc</p>
            </div>
          ) : (
            <div
              ...
              {pageNumbers.map((p) => (
                <button
                  ...
                  style={{
                    ...
                    height: 36,
                    borderRadius: 8,
                    border: p === safePage ? '1px solid #2563eb' : '1px solid #d1d5db',
                    backgroundColor: p === safePage ? '#2563eb' : '#fff',
      ...
      {mobileFilterOpen && (
        ...
        >
          ...
          >
            ...
            >
              <h2 style={{ fontSize: 18, fontWeight: 700, color: '#111827', margin: 0 }}>Bộ lọc</h2>
              <button
                onClick={() => setMobileFilterOpen(false)}
                style={{
                  display: 'flex',
