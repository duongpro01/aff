...
import { ChevronRight, ChevronLeft, Minus, Plus, ShoppingCart, Check, Truck, Shield, RotateCcw, CreditCard } from 'lucide-react';
    ...
    <div className="max-w-[1200px] mx-auto px-3 sm:px-4">
      ...
      <nav className="flex items-center gap-1 py-2.5 sm:py-3 text-[11px] sm:text-xs text-gray-500 flex-wrap leading-relaxed">
        ...
        {categorySlug && categoryName && (
          <>
            <ChevronRight size={12} className="flex-shrink-0 text-gray-400" />
            <Link href={`/products/${categorySlug}`} className="text-gray-500 no-underline whitespace-nowrap hover:text-gray-700">{categoryName}</Link>
          </>
        )}
        {brandSlug && (
          <>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 40, marginBottom: 48 }} className="product-detail-grid">
        ...
        <div style={{ minWidth: 0 }}>
          ...
          <div className="relative aspect-square rounded-lg sm:rounded-xl overflow-hidden mb-2 sm:mb-3 bg-gray-100 group">
            ...
            {discount > 0 && (
              <span className="absolute top-2 sm:top-3 left-2 sm:left-3 bg-red-600 text-white text-xs sm:text-sm font-bold px-2 sm:px-3 py-1 sm:py-1.5 rounded-md sm:rounded-lg z-10">
                -{discount}% OFF
              </span>
            )}
            {isOnSale && (
              <span className="absolute top-2 sm:top-3 right-2 sm:right-3 bg-amber-500 text-white text-[10px] sm:text-xs font-bold px-2 sm:px-2.5 py-1 rounded-md z-10">
                SALE
              </span>
            )}
